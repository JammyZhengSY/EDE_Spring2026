---
title: "Lab 3a - Workspaces, Dates, and Factors"
...
### Exercise 1: the here package in action
Use the `here` package with `read.csv()` to read the `NTL-LTER_Lake_ChemistryPhysics_Raw.csv` file into a dataframe.
...
the_year = "1865"
str_juneteenth <- paste("June, 19",the_year)
str_juneteenth
```
### The century issue
...
fix.early.dates <- function(d, cutoff) {
       ...
       m <- year(d) %% 100  #shoule become from 2026 to 26
...
fixed_date_obj_past <- fix.early.dates(date_obj_past,cutoff=26) 
...
Lubridate has some built-in fixes as well:
* `fast_strptime` requires the date format, 
* while `parse_date_time2` allows flexibility with operators like `mdy`
* both have a `cutoff_2000` parameter to specify the year cutoff
...
#Alternative 1
date_obj_past <- fast_strptime(str_past,"%y-%b-%d",cutoff_2000=26L) #use for specify the date
date_obj_past
...
Begin by resetting your R session
...
unique(sites$dow) #wed thurs frid
...
ggplot(sites,aes(x=dow)) + #indicate that the dow will bet the x-line
  geom_bar() #plot it as bar chart
```
