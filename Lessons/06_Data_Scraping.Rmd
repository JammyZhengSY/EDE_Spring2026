...
df_withdrawals <- data.frame("Month" = c(1:12), #<--NOTE: Different than recording!!
                             "Year" = rep(2000,12),
...
#Plot
ggplot(df_2020,aes(y = Avg_Withdrawals_mgd, x=Facility_name)) +
  geom_boxplot()
...
### TIDYVERSE VERSION OF ABOVE
Here we use the Tidyverse's (specifically Purrr's) `expand.grid()` and `pmap()` in place of `cross2()` and `map` for a slightly more elegant version of the above code chunk. This approach may, depending on your machine's setup, run faster as it is able to capitalize on multi-core **parallel processing**, which we can discuss in the synchronous section. 
```{r}
