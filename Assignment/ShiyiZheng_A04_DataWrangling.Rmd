---
title: "Assignment 4: Data Wrangling (Spring 2026)"
author: "Shiyi Zheng"
output: pdf_document
geometry: margin=2.54cm
editor_options: 
  chunk_output_type: console
---

## OVERVIEW

This exercise accompanies the lessons in Environmental Data Analytics on Data Wrangling.  
Do not use any AI tools in completing this assignment.  

## Directions

1. Rename this file `<FirstLast>_A04_DataWrangling.Rmd` (replacing `<FirstLast>` with your first and last name).
2. Change "Student Name" on line 3 (above) with your name.
3. Work through the steps, **creating code and output** that fulfill each instruction.
4. Be sure to **answer the questions** in this assignment document.
5. When you have completed the assignment, **Knit** the text and code into a single PDF file.
6. **Ensure that code in code chunks is tidy and does not extend off the page in the PDF**.
7. **Push your completed RMD to your GitHub account**


## Set up your session 

1a. Load the `tidyverse` and `here` packages into your session.

1b. Check your working directory.

1c. Read in all four raw data files associated with the EPA Air dataset, being sure to set string columns to be read in a factors. See the README file for the EPA air datasets for more information (especially if you have not worked with air quality data previously). 

2. Add the appropriate code to reveal the dimensions of the four datasets. 

```{r setup, message = FALSE}
#1a 
library(tidyverse)
library(here)

#1b 
getwd()
here()

#1c 
EPA_03_NC2018 <- read.csv(
  here("Data/Raw/EPAair_O3_NC2018_raw.csv"),
  stringsAsFactors = TRUE)
EPA_03_NC2019 <- read.csv(
  here("Data/Raw/EPAair_O3_NC2019_raw.csv"),
  stringsAsFactors = TRUE)
EPA_PM25_NC2018 <- read.csv(
  here("Data/Raw/EPAair_PM25_NC2018_raw.csv"),
  stringsAsFactors = TRUE)
EPA_PM25_NC2019 <- read.csv(
  here("Data/Raw/EPAair_PM25_NC2019_raw.csv"),
  stringsAsFactors = TRUE)

#2 
dim(EPA_03_NC2018)
dim(EPA_03_NC2019)
dim(EPA_PM25_NC2018)
dim(EPA_PM25_NC2019)
```

**TIP**: All four datasets should have the same number of columns but unique record counts (rows). Do your datasets follow this pattern? YES!


## Wrangle individual datasets to create processed files.

3. Change any date columns to be date objects.

4. Create new dataframes with just the following columns:  
  'Date', 'DAILY_AQI_VALUE', 'Site.Name', 'AQS_PARAMETER_DESC', 'COUNTY', 'SITE_LATITUDE', 'SITE_LONGITUDE'

5. For the PM2.5 datasets, fill all cells in AQS_PARAMETER_DESC with "PM2.5" (all cell values in this column should be identical).

6. Save all four processed datasets in the Processed folder. Use the same file names as the raw files but replace "raw" with "processed".

```{r questions 3 thru 4}
#3
library(lubridate)
EPA_03_NC2018$Date <- mdy(as.character(EPA_03_NC2018$Date))
EPA_03_NC2019$Date <- mdy(as.character(EPA_03_NC2019$Date))
EPA_PM25_NC2018$Date <- mdy(as.character(EPA_PM25_NC2018$Date))
EPA_PM25_NC2019$Date <- mdy(as.character(EPA_PM25_NC2019$Date))

#4
library(dplyr)
EPA_03_NC2018_New <- EPA_03_NC2018 %>%
  select(Date,DAILY_AQI_VALUE,Site.Name,AQS_PARAMETER_DESC,
         COUNTY,SITE_LATITUDE,SITE_LONGITUDE)

EPA_03_NC2019_New <- EPA_03_NC2019 %>%
  select(Date,DAILY_AQI_VALUE,Site.Name,AQS_PARAMETER_DESC,
         COUNTY,SITE_LATITUDE,SITE_LONGITUDE)

EPA_PM25_NC2018_New <- EPA_PM25_NC2018 %>%
  select(Date,DAILY_AQI_VALUE,Site.Name,AQS_PARAMETER_DESC,
         COUNTY,SITE_LATITUDE,SITE_LONGITUDE)

EPA_PM25_NC2019_New <- EPA_PM25_NC2019 %>%
  select(Date,DAILY_AQI_VALUE,Site.Name,AQS_PARAMETER_DESC,
         COUNTY,SITE_LATITUDE,SITE_LONGITUDE)
#5
EPA_PM25_NC2018_New$AQS_PARAMETER_DESC <- "PM2.5"
EPA_PM25_NC2019_New$AQS_PARAMETER_DESC <- "PM2.5"

#6
write.csv(
  EPA_03_NC2018_New,
  file = here("Data/Processed/EPAair_O3_NC2018_processed.csv"),
  row.names = FALSE
)

write.csv(
  EPA_03_NC2019_New,
  file = here("Data/Processed/EPAair_O3_NC2019_processed.csv"),
  row.names = FALSE
)

write.csv(
  EPA_PM25_NC2018_New,
  file = here("Data/Processed/EPAair_PM25_NC2018_processed.csv"),
  row.names = FALSE
)

write.csv(
  EPA_PM25_NC2019_New,
  file = here("Data/Processed/EPAair_PM25_NC2019_processed.csv"),
  row.names = FALSE
)
```


## Combine datasets

7. Combine the four datasets with `rbind`. Make sure your column names are identical prior to running this code.

8. Use code to display the dimensions of the combined dataset.

9. Wrangle your new dataset with a pipe function (%>%) so that it fills the following conditions: 

  * Include only sites that the four data frames have in common:  
    "Linville Falls", "Durham Armory", "Leggett", "Hattie Avenue",  
    "Clemmons Middle", "Mendenhall School", "Frying Pan Mountain", 
    "West Johnston Co.", "Garinger High School", "Castle Hayne", 
    "Pitt Agri. Center", "Bryson City", "Millbrook School" 

  * Some sites have multiple measurements per day. Use the split-apply-combine strategy to generate daily means: group by date, site name, AQS parameter, and county. Take the mean of the AQI value, latitude, and longitude.
  
  * Add new columns for "Month" and "Year" by parsing your "Date" column (hint: `lubridate` package)
  
  * Hint: the dimensions of this dataset should be 14,752 x 9.

10. Spread your datasets such that AQI values for ozone and PM2.5 are in separate columns. Each location on a specific date should now occupy only one row.

11. Call up the dimensions of your new tidy dataset.

12. Save your processed dataset with the following file name: "EPAair_O3_PM25_NC1819_Processed.csv"

```{r questions 7 thru 12}
#7 
EPA_all <- rbind(
  EPA_03_NC2018_New,
  EPA_03_NC2019_New,
  EPA_PM25_NC2018_New,
  EPA_PM25_NC2019_New
)

#8 
dim(EPA_all)

#9

library(lubridate)
library(dplyr)

site_common <- c("Linville Falls", "Durham Armory", "Leggett", "Hattie Avenue",  
    "Clemmons Middle", "Mendenhall School", "Frying Pan Mountain", 
    "West Johnston Co.", "Garinger High School", "Castle Hayne", 
    "Pitt Agri. Center", "Bryson City", "Millbrook School")

EPA_daily <- EPA_all %>%
  mutate(
    Site.Name = trimws(as.character(Site.Name)),
    COUNTY = trimws(as.character(COUNTY)),
    DAILY_AQI_VALUE = as.numeric(DAILY_AQI_VALUE),
    SITE_LATITUDE = as.numeric(SITE_LATITUDE),
    SITE_LONGITUDE = as.numeric(SITE_LONGITUDE)) %>%
  filter(Site.Name %in% site_common) %>%
  group_by(Date,Site.Name,AQS_PARAMETER_DESC,COUNTY) %>%
  summarise(
    DAILY_AQI_VALUE = mean(DAILY_AQI_VALUE,na.rm = TRUE),
    SITE_LATITUDE = mean(SITE_LATITUDE,na.rm = TRUE),
    SITE_LONGITUDE = mean(SITE_LONGITUDE,na.rm = TRUE),
    .groups = "drop") %>%
  mutate(
    Month = month(Date), 
    Year = year(Date))

  
dim(EPA_daily)

#10
EPA_tidy <- EPA_daily %>%
  select(Date, Site.Name, COUNTY, SITE_LATITUDE, SITE_LONGITUDE, Month, Year,
         AQS_PARAMETER_DESC, DAILY_AQI_VALUE) %>%
  pivot_wider(names_from  = AQS_PARAMETER_DESC,
              values_from = DAILY_AQI_VALUE)

#11
dim(EPA_tidy)

#12
write.csv(
  EPA_tidy,
  file = "EPAair_O3_PM25_NC1819_Processed.csv",
  row.names = FALSE)

```


## Generate summary tables

13. Use the split-apply-combine strategy to generate a summary data frame. Data should be split into groups by site, month, and year. Then compute the mean AQI values for ozone and PM2.5 for each group. Finally, add a pipe to remove instances where the mean **ozone** values are not available (use the function `drop_na` in your pipe). It's ok to have missing mean PM2.5 values in this result. 

14. Call up the dimensions of the summary dataset.

```{r questions 13 and 14}

#13
EPA_summary <- EPA_tidy %>%
  group_by(Site.Name, Month, Year) %>%
  summarise(mean_Ozone = mean(Ozone, na.rm = TRUE),
            mean_PM25  = mean(`PM2.5`, na.rm = TRUE),
            .groups = "drop") %>%
  drop_na(mean_Ozone)

#14
dim(EPA_summary)

```

15. Why did we use the function `drop_na` rather than `na.omit`? Hint: replace `drop_na` with `na.omit` in part 13 and observe what happens with the dimensions of the summary date frame.

> Answer: We used drop_na because it allows us to remove rows based only on missing values in a specific variable—in this case, the mean ozone AQI—while retaining rows where mean PM2.5 values are missing. This is important because the assignment explicitly allows missing PM2.5 values in the summary dataset.In contrast, na.omit removes any row that contains at least one missing value across all variables. When na.omit is used, rows with missing mean PM2.5 values are also removed, even if the mean ozone value is available. As a result, the dimensions of the summary dataset become smaller and do not reflect the intended grouping structure. Therefore, drop_na is more appropriate for selectively filtering missing data without unnecessarily discarding valid observations.

16. Stage, commit, and push your Assignment to your GitHub account. Provide a link to your repository below.

> Github repository URL: https://github.com/JammyZhengSY/DATA-Exploration
